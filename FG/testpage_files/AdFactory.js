...
function analyer(){this.sentence='';this.get_by_id=function(){if(document.getElementById('fga')!=null){var obj=document.getElementById('fga');this.go(obj.innerHTML);}}
this.send_text=function(t){this.sentence=t;this.go(this.sentence);}
this.get_by_useid=function(id){if(document.getElementById(id)!=null){var obj=document.getElementById(id);this.go(obj.innerHTML);}}
this.go=function(t){channel=document.domain;channel=channel.split('.')[0];ap="http://54.248.90.126/";pa="Chan="+channel+"&t="+t+"&rnd="+Math.floor(Math.random()*1000000);xhr=false;xdr=false;if(window.XDomainRequest){try{xdr=new XDomainRequest();xhr=false;}catch(e){xdr=false;}}else if(window.XMLHttpRequest){try{xhr=new XMLHttpRequest();}catch(e){xhr=false;}}else if(window.ActiveXObject){try{xhr=new ActiveXObject("Msxml2.XMLHTTP");}catch(e){try{xhr=new ActiveXObject("Microsoft.XMLHTTP");}catch(e){xhr=false;}}}
if(xhr){xhr.open("POST",ap,true);xhr.setRequestHeader('Content-Type','application/x-www-form-urlencoded');xhr.onreadystatechange=function(){if(xhr.readyState===4){if(xhr.status===200){if(xhr.responseText=="false"){console.log(xhr.responseText);}else{track_visit(xhr.responseText,0);set_favorite_cookie();}}}};xhr.send(pa);}
if(xdr){xdr.onerror=function(){}
xdr.onload=function(){if(xdr.responseText=="false"){}else{track_visit(xdr.responseText,0);set_favorite_cookie();}}
